...
function QuizPage() {
  ...
  useEffect(() => {
    if (user && user.telegram_id) {
      if (!user.cargo || !user.canal_principal) {
        setProfileError("Seu perfil está incompleto. Por favor, atualize seu cargo e canal principal para poder jogar.");
        return; 
      }
      ...
      startQuiz(params, navigate);
    }
    
    return () => {
      resetQuiz();
    };
  }, [user, searchParams, startQuiz, resetQuiz, navigate]);

  const getButtonClassName = (option) => {
    if (!selectedAnswer) return styles.answerOption;
    const currentQuestion = quizData.questions[currentQuestionIndex];
    const isCorrectAnswer = option === currentQuestion.correta;
  ...
  // --- CORREÇÃO APLICADA ---
  // Função que renderiza o texto da pergunta, aplicando negrito aos marcadores "a)", "b)", etc.
  const renderQuestionWithBoldMarkers = (text) => {
    if (!text) return null;
    return text.split('\n').map((line, index) => {
      // Procura por linhas que começam com um marcador como "a) "
      ...
      // Linhas sem marcador (a pergunta principal) são renderizadas normalmente
      return <div key={index} className={styles.questionLine}>{line}</div>;
    });
  };
  ...
  if (profileError) {
    return (
      <div className={styles.errorContainer}>
        ...
        <p>{profileError}</p>
        <button onClick={() => navigate('/profile')} className={styles.errorButton}>
          Completar Perfil
        </button>
      </div>
    );
        ...
        <div className={styles.quizLayout}>
            <div className={styles.questionArea}>
                <div className={styles.questionText}>
                    {/* Usa a nova função para renderizar o texto com os marcadores em negrito */}
                    {renderQuestionWithBoldMarkers(currentQuestion.pergunta_formatada_display)}
                </div>
            </div>
